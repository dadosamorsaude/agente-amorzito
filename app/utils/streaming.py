...
async def _run_evaluation_background(
    ...
    """Executa a avaliação em background sem bloquear o fluxo principal."""
    try:
        logger.info(
...
async def stream_agent_response(
    ...
    user_id: str,
    stream: bool,
    athena_results_context: Any,
    rag_results_context: Any,
        ...
        if stream:
            ...
            try:
                ...
                if (raw_data or rag_data) and final_response:
                    history_str = "\n".join([f"{type(m).__name__}: {m.content}" for m in input_messages[:-1]])
                    asyncio.create_task(
